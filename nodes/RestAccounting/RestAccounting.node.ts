...
async function apiRequest(
    ...
    try {
        ...
        throw new NodeApiError(this.getNode(), { message: (error as any)?.message || 'Request failed' });
    }
}

export class RestAccounting implements INodeType {
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (resource === 'accounts') {
                ...
                if (operation === 'create') {
                    const body = this.getNodeParameter('accountFields', i, {}) as IDataObject;
            ...
            if (resource === 'journals') {
                ...
                if (operation === 'create') {
                    const body = this.getNodeParameter('journalPayload', i) as IDataObject;
                    response = await apiRequest.call(this, 'POST', '/journal-entries', body);
                }
                if (operation === 'get') {
                    const id = this.getNodeParameter('entryId', i) as number;
            ...
            if (resource === 'reports') {
                ...
                if (operation === 'ledger') {
                    const accountId = this.getNodeParameter('ledgerAccountId', i) as number;
